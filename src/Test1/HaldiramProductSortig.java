...
public class HaldiramProductSortig {
	...
	public void CheckPrice() throws InterruptedException 
	{
		...
		List<WebElement> beforeFilter = driver.findElements(By.xpath("//div[@class='product-cards__item--name']"));
...
//		Thread.sleep(3000);
		...
		for(WebElement pr: afterDesendingFilter)
		{
			...
			afterFilterPriceList.add(String.valueOf(pr.getText()));
	...
	public void asendingOrederFilter() throws InterruptedException
	{
				...
				List<WebElement> afterDesendingFilter = driver.findElements(By.xpath("//div[@class='product-cards__item--name']"));
				Thread.sleep(3000);
				...
				for(WebElement pr: afterDesendingFilter)
				{
					...
					afterFilterPriceList.add(String.valueOf(pr.getText().replace("₹","").replace(",", "")));
